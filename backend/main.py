...
@app.get("/pdf/lunar")
def pdf_lunar(luna: int, an: int, token: str, current_user=None): # Acceptăm token direct
    db = get_db()
    # Verificăm manual sesiunea deoarece token-ul vine în URL
...
@app.get("/pdf/fisa/{id}")
def pdf_fisa(id: int, token: str, current_user=None): # Acceptăm token direct
    db = get_db()
    user = crud.get_session_user(db, token)
    if not user:
        raise HTTPException(status_code=401, detail="Sesiune invalidă")
